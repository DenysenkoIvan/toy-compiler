...
FunctionDeclaration::FunctionDeclaration(Type ret_type, std::string&& id, std::vector<std::unique_ptr<VariableDeclaration>>&& args, std::vector<std::unique_ptr<Statement>>&& stmts)
	: m_ret_type(ret_type), m_id(std::move(id)), m_arguments(std::move(args)), m_statements(std::move(stmts)) {}
